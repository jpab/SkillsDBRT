(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('skillsDbrtApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-skillsDbrtApp-error');
                var entityKey = httpResponse.headers('X-skillsDbrtApp-params');
                if (errorHeader) {
